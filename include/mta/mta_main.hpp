...
extern inline ILuaModuleManager10* pModuleManager = nullptr;
extern inline fs::path MTAPath = fs::path();
extern inline fs::path ResourcesPath = fs::path();

inline std::string GetResourceName(lua_State* luaVM) noexcept {
...
inline void* GetRootElement(lua_State* luaVM) noexcept {
	lua_getglobal(luaVM, "root");
	return lua_touserdata(luaVM, -1);
}
inline void* GetResourceRootElement(lua_State* luaVM) noexcept {
